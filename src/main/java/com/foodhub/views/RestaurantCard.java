...
public class RestaurantCard extends VBox implements Initializable {
	...
	private Pane image;
	@FXML
	private Label name, types, address, deliveryTime;
	@FXML
	private RatingView rating;
	@FXML
	private Label votes;
	private final ObjectProperty<Restaurant> restaurant = new SimpleObjectProperty<>(this, "restaurant");
	...
	private void updateViews() {
		...
		if (getRestaurant().getImageUrl() != null)
		...
		if (getRestaurant().getDeliveryTime() != null)
			deliveryTime.setText(String.format("%.0f Mins", getRestaurant().getDeliveryTime().toMinutes()));
